...
export class FlipDown {
  ...
  _createRotorGroup(rotors, rotorIndex) {
    ...
    if (this.opts.showHeader) {
      ...
      dayRotorGroupHeading.setAttribute('data-after', this.opts.headings[rotorIndex - 1]);
    } else {
      // insert blank text to avoid css fail
      dayRotorGroupHeading.setAttribute('data-before', ' ');
      dayRotorGroupHeading.setAttribute('data-after', ' ');
  ...
  _updateClockValues(init = false, _reset = false) {
    ...
    if (this.opts.showHour == 'auto' && (this.clockValues.h > 0 || this.state == 'idle')) {
      if (!this.headerShift) {
        this.rotorGroups.forEach((el) => el.classList.add('autohour'));
        this.headerShift = true;
      }
      ...
      this.clockValuesAsString = ('00' + this.clockStrings.h + this.clockStrings.m).split('');
    } else {
      if (this.headerShift) {
        this.rotorGroups.forEach((el) => el.classList.remove('autohour'));
        this.headerShift = false;
      }
    ...
    function rotorRev() {
      this.rotorLeafFront.forEach((el, _i) => {
        el.classList.add('front-bottom');
      });
      this.rotorLeafRear.forEach((el, _i) => {
